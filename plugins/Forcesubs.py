    ...
    if UPDATE_CHANNEL:
        try:
            ...
            if user.status == "kicked":
               await m.reply_text("**Hey you are banned from our official channel😜**", quote=True)
               return
        except UserNotParticipant:
            buttons = [[InlineKeyboardButton(text='Updates Channel 🔖', url=f"https://t.me/bryll_education")]]
            if m.text:
            ...
            await m.reply_text(
                f"Hey {m.from_user.mention(style='md')} you need join My owner's channel in order to use me 😉\n\n"
...
async def refresh_cb(c, m):
    ...
    if UPDATE_CHANNEL:
        try:
            ...
            if user.status == "kicked":
               try:
                   await m.message.edit("**Hey you are banned from our official channel😜**")
               except:
                   pass
               return
        except UserNotParticipant:
            await m.answer('You are not yet joined our offical channel. First join and then press refresh button 🤤', show_alert=True)
            return
        except Exception as e:
            print(e)
